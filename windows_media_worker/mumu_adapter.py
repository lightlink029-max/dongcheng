...
def _candidate_roots():
    ...
    roots.extend((Path("D:/Program Files/Netease/MuMuPlayer-12.0"), Path("D:/MuMuPlayer-12.0")))
    return roots
...
def find_adb(configured=""):
    ...
    if system_adb:
        return system_adb
    relative_paths = ("shell/adb.exe", "adb.exe")
...
def find_player(configured=""):
    ...
    for root in _candidate_roots():
        for name in ("shell/MuMuPlayer.exe", "MuMuPlayer.exe", "nx_main/3.0/MuMuPlayer.exe"):
            candidate = root / name
            if candidate.is_file():
                return str(candidate)
    return ""
...
class MumuBridge:
    def __init__(self, adb_path="", serial=DEFAULT_SERIAL, player_path="", runner=None):
        ...
        self.serial = (serial or DEFAULT_SERIAL).strip()
...
def check_mumu(config):
    bridge = MumuBridge(
        config.get("mumu_adb", ""), config.get("mumu_serial", DEFAULT_SERIAL),
