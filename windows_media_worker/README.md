...
原始素材和分镜素材统一使用四组标签：经营角色、业务场景、分镜用途和自定义标签。下载或上传后可单条、批量打标；另存初剪片段时自动继承原素材标签；加工完成进入分镜库后仍可独立重新打标，不反向修改原素材。制作分镜时可按标签筛选全局原始素材；选择成片时可按标签、关键词以及当前视频方案的画面要求筛选，默认把完全匹配和部分匹配素材排在前面，但最终关联必须由用户确认。
