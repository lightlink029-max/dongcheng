...
基础剪辑支持顺序、倒序和随机拼接，可保留原声或静音，可选择不加字幕、使用项目脚本或识别原视频语音。`whisper_command` 是可选的本地语音识别程序路径，程序需按“输入视频、输出 SRT、原视频语种”的顺序接收三个参数。`ai_edit_command` 是可选的本地 AI 剪辑程序路径，程序接收输入 JSON 和输出 JSON 两个参数，输出 `{"order": [0, 1]}` 形式且必须包含全部素材索引。未配置这些程序时，基础剪辑和脚本字幕仍可正常工作。
