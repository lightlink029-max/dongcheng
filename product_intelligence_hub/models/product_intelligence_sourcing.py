...
class ProductIntelligenceSourcingOffer(models.Model):
    ...
    @api.depends("image_url", "image_512")
    def _compute_image_preview(self):
        for offer in self:
            url = offer.image_url
            if not url and offer.id and offer.image_512:
                url = "/web/image/product.intelligence.sourcing.offer/%s/image_512" % offer.id
            escaped_url = html.escape(url or "", quote=True)
            offer.image_preview = (
                '<img src="%s" alt="1688货源主图" '
                'style="width:80px;height:80px;object-fit:contain;border-radius:6px"/>' % escaped_url
                if escaped_url else ""
            )
