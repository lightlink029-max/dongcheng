...
class ProductIntelligenceCandidate(models.Model):
    ...
    def _prepare_ecommerce_description(self, existing_description=None):
        ...
        sections = [
            ("important", _("重要属性"), self.important_attributes),
            ("packaging", _("包装信息"), self.packaging_information),
            ("shipping", _("发货及交货时间"), self.shipping_information),
        ]
        cards = []
        for section_key, title, content in sections:
            rows = []
            ...
            if rows:
                body_class = "card-body py-1"
                if section_key == "important":
                    body_class += " pi-important-attributes-grid"
                cards.append(
                    ...
                    f'<div class="{body_class}">{"".join(rows)}</div>'
                    '</section></div>'
                )
        managed_block = ""
        if cards:
    ...
    def _prepare_product_values(self, product=None):
        ...
        return {
            ...
            # The sourced detail area is fully managed so every product keeps
            # the same three-section website layout without legacy duplicates.
