...
class ProductIntelligenceIngestController(http.Controller):
    ...
    @http.route(
        "/product-intelligence/v1/supplier-contact-queue/<int:source_id>", type="http",
        auth="public", methods=["GET"], csrf=False, save_session=False,
    )
    def supplier_contact_queue(self, source_id, **kwargs):
        source = self._authorized_source(source_id)
        if not source:
            return request.make_json_response({"ok": False, "error": "unauthorized"}, status=401)
        offers = request.env["product.intelligence.sourcing.offer"].sudo().search([
            ("candidate_id.company_id", "=", source.company_id.id),
            ("platform", "=", "yiwugo"),
            ("contact_enrichment_state", "=", "queued"),
            ("supplier_url", "!=", False),
        ], order="write_date asc", limit=50)
        items, seen = [], set()
        for offer in offers:
            if offer.supplier_url in seen:
                continue
            seen.add(offer.supplier_url)
            items.append({"offer_id": offer.id, "supplier_name": offer.supplier_name, "supplier_url": offer.supplier_url})
            if len(items) >= 5:
                break
        return request.make_json_response({"ok": True, "items": items})

    @http.route(
        "/product-intelligence/v1/supplier-contact-result/<int:source_id>", type="http",
        auth="public", methods=["POST"], csrf=False, save_session=False,
    )
    def supplier_contact_result(self, source_id, **kwargs):
        source = self._authorized_source(source_id)
        if not source:
            return request.make_json_response({"ok": False, "error": "unauthorized"}, status=401)
        try:
            payload = json.loads(request.httprequest.get_data(as_text=True) or "{}")
            offer = request.env["product.intelligence.sourcing.offer"].sudo().browse(int(payload.get("offer_id") or 0)).exists()
            if not offer or offer.candidate_id.company_id != source.company_id:
                return request.make_json_response({"ok": False, "error": "offer_not_found"}, status=404)
            matching = request.env["product.intelligence.sourcing.offer"].sudo().search([
                ("candidate_id.company_id", "=", source.company_id.id),
                ("supplier_url", "=", offer.supplier_url),
            ])
            values = {
                "contact_name": str(payload.get("contact_name") or "")[:256],
                "contact_phone": str(payload.get("contact_phone") or "")[:128],
                "contact_landline": str(payload.get("contact_landline") or "")[:128],
                "contact_email": str(payload.get("contact_email") or "")[:256],
                "contact_qq": str(payload.get("contact_qq") or "")[:256],
                "contact_details": str(payload.get("contact_details") or "")[:2000],
                "contact_enrichment_state": "done",
            }
            matching.write(values)
            partners = matching.mapped("supplier_partner_id")
            if partners:
                partner_values = {
                    "phone": values["contact_phone"] or values["contact_landline"],
                    "email": values["contact_email"],
                }
                partner_values = {key: value for key, value in partner_values.items() if value}
                if partner_values:
                    partners.write(partner_values)
                contact_note = "\n".join(filter(None, [
                    f"联系人：{values['contact_name']}" if values["contact_name"] else "",
                    f"座机：{values['contact_landline']}" if values["contact_landline"] else "",
                    f"QQ：{values['contact_qq']}" if values["contact_qq"] else "",
                    values["contact_details"],
                ]))
                if contact_note:
                    for partner in partners:
                        if contact_note not in (partner.comment or ""):
                            partner.comment = "\n".join(filter(None, [partner.comment or "", contact_note]))
            return request.make_json_response({"ok": True, "updated": len(matching)})
        except Exception:
            _logger.exception("Supplier contact update failed for source %s", source_id)
            return request.make_json_response({"ok": False, "error": "internal_error"}, status=500)
