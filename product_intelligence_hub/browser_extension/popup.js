...
document.querySelector('#push').addEventListener('click', async () => {
  try {
    ...
    const isYiwugo=/^https:\/\/([a-z0-9-]+\.)*yiwugo\.com\/search/i.test(tab.url||'');
    let result;
    if(is1688||isYiwugo){
      ...
      try{
        result=await chrome.tabs.sendMessage(tab.id,{type:isYiwugo?YIWUGO_MESSAGE:SOURCING_MESSAGE});
      }catch(error){
        await chrome.scripting.executeScript({target:{tabId:tab.id},files:['content.js']});
        result=await chrome.tabs.sendMessage(tab.id,{type:isYiwugo?YIWUGO_MESSAGE:SOURCING_MESSAGE});
      }
      if(!result?.candidate_id){
        const saved=await chrome.storage.local.get([isYiwugo?'lastYiwugoCandidateId':'last1688CandidateId']);
        result ||= {items:[]};
        result.candidate_id=Number(saved[isYiwugo?'lastYiwugoCandidateId':'last1688CandidateId']||0);
      }
      if(!result?.candidate_id) throw new Error('请从 Odoo 产品机会的货源研判页面进入当前搜索页');
    }else{
